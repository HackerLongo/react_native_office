...
export default class IApprovalList extends React.Component {
  constructor(props){
    super(props);
    this.state = {
  ...
  fetchData() {
    var url = api.OFFICE_IAPPROVALLIST + 'userId=' + this.props.router.userId;
    console.log("url = " + url);
    ...
    if(this.state.loaded)
    {
      return(
        <View style={styles.background}>
          <NavigationBar title={'已办任务'} titleColor={Colors.white}
      ...
      <TouchableOpacity  onPress={() => this.onDetail(rowData)} >
        ...
        <View style={styles.leftView}>
          <View style={styles.topText}>
            <Text style={styles.textTitle} numberOfLines={1}>{rowData.processNo}--{rowData.processTitle}</Text>
          </View>
          <View style={styles.bottomText}>
            <Text style={styles.userName } numberOfLines={1}>申请人:{rowData.applicantName}</Text>
            <Text style={styles.currentTitle} numberOfLines={1}>{rowData.status}</Text>
            <Text style={styles.timeTitle }>{rowData.startTime}</Text>
          </View>
        </View>
